<?php
  ...
  if($_SESSION['access'] == 'super' ){
    ...
    <?php
  } else if ($_SESSION['access'] == 'agent'){
    ?>
      <script>
        window.location.href = 'agent_daily.php';
      </script>
...
if(!isset($_SESSION['access'])){
  ...
  $sql = "select * from sv where month = '$current_month' and year = '$current_year'";
  $result = $connect->query($sql);
  $row = $result->fetch_assoc();
  if($result->num_rows > 0){

    $sql_current = "select * from sv where month = '$current_month' and year = '$current_year' and queue = '$queue'";
 

  

  }else{
    $sql_last = "select * from sv where queue = '$queue' order by id desc  ";
    $result_last = $connect->query($sql_last);
    $row_last = $result_last->fetch_assoc();
    $month = $row_last['month'];
    $year = $row_last['year'];

    $sql_current = "select * from sv where month = '$month' and year = '$year' and queue = '$queue' ";
          ...
          <div class="col-lg-6 col-md-12">
            <div class="card" style="width: 215%;margin-left: -15px;">
              <div class="card-header">
                <h4 class="card-title"> SPV Score</h4>
              </div>
              <div class="card-body">
                      ...
                      <?php
                        $result_current = $connect->query($sql_current);
                         while($row_current = $result_current->fetch_assoc()){
                          ...
                          ?>
                          <tr class="text-center">
                            <td><a href="agent_details.php?id=<?php echo $row_current['sv_id'];?> "><?php echo $row_current['name']; ?>                          </a></td>
