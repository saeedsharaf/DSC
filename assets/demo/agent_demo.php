<?php
      ...
      include'../config/connect.php';
       ...
       $day = "select * from dashboard where queue = 'pre' and month = '$month' order by id desc limit 10"; //////// to get last 10 day's
  ...
  //$connect = mysqli_connect('localhost','root','','dsc');
          ...
          global $login_id;
          global $month;
            $day1 = "select * from agent_daily where queue = 'pre' and login_id = '$login_id' and month = '$month'  "; /// //////// to get last 10 day's ignore pre
            $day_result1 = $connect->query($day1);
            $days1 = 0;
            if ($day_result1->num_rows > 0 ){
              

            while($day_row1 = $day_result1->fetch_assoc()){
              $date1 = date_create($day_row1['day']);
...
///////////////////////////////////////// hold //////////////////////////////////////////
   ...
   var sl_day = [
    ...
    <?php
       ...
       $hold = "select * from agent_daily where login_id = '$login_id' and month = '$month'";
            $hold_result = $connect->query($hold);
            if ($hold_result->num_rows > 0 ){
               $x = $hold_result->num_rows ;

                while($hold_row = $hold_result->fetch_assoc()){
                   $hold = $hold_row['hold'] ;
                  if($x > 10){
                    $x--;
                    continue ;
...
<?php
$connect = mysqli_connect('localhost','root','','dsc');
$acw = "select * from agent_daily where login_id = '$login_id' and month = '$month' ";
      $acw_result = $connect->query($acw);
            if ($acw_result->num_rows > 0 ){

              $h = $acw_result->num_rows ;   // to count result number

                while($acw_row = $acw_result->fetch_assoc()){

                   $acw_score = $acw_row['acw'];
                   if(is_nan($acw_score)){
                    $acw_score = '';
...
///////////////////////////////////////////////  nps   ////////////////////////////////////
    ...
    var chart_data = [
    <?php
    $nps = "select * from agent_daily where login_id = '$login_id' and month = '$month'";
...
<?php
            ...
            $aht = "select * from agent_daily where login_id = '$login_id' and month = '$month'";
